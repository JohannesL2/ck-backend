...
const ENV = ENV_TEXT.startsWith("prod")
  ? environments.PROD
  : ENV_TEXT.startsWith("stage")
  ? environments.STAGE
  : ENV_TEXT.startsWith("test")
  ? environments.TEST
  : environments.DEV;
...
const transports = [
  ...
  ...(LOG_FILE
    ? [new winston.transports.File({ filename: LOG_FILE, level: LOG_LEVEL })]
    : []),
];
